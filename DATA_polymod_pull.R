...
d.merge<-select(merged,global_id,local_id.x,country.x,participant_age,dayofweek, cnt_count.x,cnt_age_mean,cnt_home,cnt_leisure,cnt_work,cnt_school,cnt_transport,cnt_otherplace,cnt_touch, holiday) 

#country levels BE DE FI GB IT LU NL PL
BE<-subset(subset(d.merge,country.x=='BE'),touchtype)
DE<-subset(subset(d.merge,country.x=='DE'),touchtype)
FI<-subset(subset(d.merge,country.x=='FI'),touchtype)
GB<-subset(subset(d.merge,country.x=='GB'),touchtype)
IT<-subset(subset(d.merge,country.x=='IT'),touchtype)
LU<-subset(subset(d.merge,country.x=='LU'),touchtype)
NL<-subset(subset(d.merge,country.x=='NL'),touchtype)
PL<-subset(subset(d.merge,country.x=='PL'),touchtype)
...
{
merge<-combo[[kk]];
workdata<-!is.na(merge$dayofweek)&!is.na(merge$cnt_age_mean)&!is.na(merge$participant_age)&!is.na(merge$holiday)
...
###For AGE mean
na.omit(workdata$cnt_age_mean)
which(is.na(workdata$cnt_age_mean))
for (i in 1:length(workdata$cnt_age_mean))
{ 
  workdata$age_group_mean[i]<-as.age.group(workdata$cnt_age_mean[i], limits = as.numeric(c(1, 5, 15, 25, 45, 65)));
}
...
if(kk==5) {agegrp1<-as.data.frame(rep(0,dim(B)[1]));
B<-cbind(agegrp1,B);
colnames(B)=c('agegrp1','agegrp2','agegrp3','agegrp4','agegrp5','agegrp6','agegrp7')}
#####
workdata<-cbind(workdata,B)
GBfinal<-select(workdata,global_id,local_id.x,participant_age,agegrp1,agegrp2,agegrp3,agegrp4,agegrp5,agegrp6,agegrp7,day)

#options(na.action='na.omit')
set<-ddply(GBfinal,c("global_id","local_id.x","participant_age","day"),numcolwise(sum))
set<-set[complete.cases(set),];
set$participant_age<-na.pass(as.numeric(as.character(set$participant_age)))
set$day<-na.pass(as.numeric(as.character(set$day)))
set<-na.omit(set)
colnames(set)=c('V0','V1','V2','V3','V4','V5','V6','V7','V8','V9','V10')

newpolymod<-set[,3:11]
colnames(newpolymod)=c('V1','V2','V3','V4','V5','V6','V7','V8','V9')
newpolymod<-arrange(newpolymod, newpolymod$V1)

sname<-c('BEtable', 'DEtable', 'FItable', 'GBtable', 'ITtable', 'LUtable', 'NLtable', 'PLtable')

write.csv(newpolymod,paste0(sname[kk],".csv"),row.names = FALSE)
  }
}
#set mismatch check
